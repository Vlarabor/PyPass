...
def convert_to_bytes(v: int or str) -> bytes:
    ...
    ------
    IOError
        If the given value is not of type int or str.
    """
    if type(v) is str:
        return v.encode("utf-8")
    elif type(v) is int:
        return v.to_bytes(sys.getsizeof(v), sys.byteorder)
    else:
        raise IOError("v must be of type int or str.")


def convert_to_int(b: bytes) -> int:
    """
    Converts the given byte value into an integer.
...
def convert_to_string(b: bytes) -> str:
    """
    Converts the given byte value into a string.
...
def get_group_params(group_bytes_size: GroupByteSize) -> Tuple[int, int]:
    """
    Get the group parameters N and g of the given size.

    Parameters
    ----------
    group_bytes_size : GroupByteSize
...
def get_random_number(n_bytes: int) -> int:
    """
    Get a random number of specified size that can be used for cryptography.
...
def hash_args(hash_func, *args) -> int:
    ...
    for s in args:
        if s is not None:
            data = convert_to_bytes(s) if type(s) is int or type(s) is str else s
            h.update(data)
